...
const DropdownMenu = ({ items, onClose }) => (
  <div role="menu" className="absolute top-full left-0 mt-0 w-56 bg-white shadow-xl border-t-2 border-[#2563EB] border-x border-b border-[#BFDBFE] py-1 z-50">
    {items.map((item) => (
      <Link key={item.path} to={item.path} role="menuitem" onClick={onClose}
        className="block px-4 py-2.5 text-sm text-[#1E293B] hover:bg-[#EFF6FF] hover:text-[#1B4F8A] transition-colors focus:outline-none focus:bg-[#EFF6FF]">
        {item.name}
      </Link>
    ))}
  </div>
...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 shadow-lg" ref={navRef}>
      ...
      {/* ROW 2 — College name full-width banner */}
      <div className="bg-[#1B4F8A] text-white w-full">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
          <Link to="/"
            className="flex items-center gap-4 focus:outline-none focus:ring-2 focus:ring-[#F59E0B] rounded-lg p-1 flex-1"
            aria-label="Swami Vivekanand College – Home">
            <div className="shrink-0 bg-white rounded-full p-1.5 shadow-lg border-2 border-[#F59E0B]">
              <img
                src={kksuLogo}
                alt="College Logo"
            ...
            <div>
              <p className="text-[#F59E0B] text-xs md:text-sm font-semibold tracking-wide mb-1">
                Managed by Shri Shamki Mata Education, Arts, Sports and Health Multipurpose Organization, Akola.
              </p>
              <h1 className="font-extrabold text-xl md:text-2xl lg:text-3xl leading-tight tracking-wide text-white uppercase">
                Swami Vivekanand College of Arts and Science
              </h1>
              <p className="text-[#BFDBFE] text-xs md:text-sm font-medium tracking-widest mt-0.5">
                ...
                <div key={item.path} className="relative">
                  {item.children ? (
                    <button
                      ...
                      className={`flex items-center gap-1 px-4 py-3.5 text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[#F59E0B] focus:ring-inset border-b-2 ${
                        isActive(item.path)
                    ...
                    <Link to={item.path}
                      className={`block px-4 py-3.5 text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[#F59E0B] focus:ring-inset border-b-2 ${
                        isActive(item.path)
                          ? 'border-[#1B4F8A] text-[#1B4F8A] bg-blue-50'
                          : 'border-transparent text-black hover:text-[#1B4F8A] hover:bg-gray-100 hover:border-[#1B4F8A]'
                      }`}
              ...
              {searchOpen ? (
                <form onSubmit={handleSearch} className="flex items-center gap-1">
                  ...
                  <button type="submit" aria-label="Submit search"
                    className="p-1.5 bg-[#2563EB] text-white rounded hover:bg-[#1D4ED8] transition-colors focus:outline-none focus:ring-2 focus:ring-[#F59E0B]">
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <nav id="mobile-menu" className="lg:hidden bg-[#1B4F8A] border-t border-white/10" aria-label="Mobile Navigation">
                ...
                {item.children ? (
                  <>
                    <button onClick={() => setOpenDropdown(openDropdown === item.name ? null : item.name)}
                      className="w-full flex items-center justify-between px-3 py-2.5 text-sm font-medium text-[#BFDBFE] hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#F59E0B] rounded">
                      {item.name}
                      ...
                      <div className="pl-4 mt-0.5 space-y-0.5 border-l-2 border-[#F59E0B]/50 ml-3">
                        {item.children.map((child) => (
                          <Link key={child.path} to={child.path}
                            ...
                            className="block px-3 py-2 text-sm text-[#BFDBFE] hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-[#F59E0B] rounded">
                            {child.name}
                          </Link>
                        ))}
                      </div>
                  ...
                  <Link to={item.path} onClick={() => setIsOpen(false)}
                    className={`block px-3 py-2.5 text-sm font-medium rounded focus:outline-none focus:ring-2 focus:ring-[#F59E0B] ${
